...
> **⚠️ 3a-5 재보정 (2026-09, 사용자 승인)** — 아래 필터 표는 원안. 라이브 검증 결과
> LOW/MID 일부 임계값이 현 시장 대비 과도하게 엄격해 다음과 같이 완화됐다.
> **운영 기준값은 `config/filters/*.yaml` (근거 주석 포함), 이력은 `docs/reviews/3a-5.md`.**
> - LOW β 하한 0.6→0.15, dgr_5y 5%→3%, eps_payout 상한 65%→75%, fcf_payout 상한 75%→85%
...
### B-3.1. 종목 사이징 — 카테고리 예산 → 개별 종목 (결정론 코어)

> **확정 2026-09-01** (3a-8 착수 시 결정). `PositionSizer` / `pipeline.py` 구현 근거.
> 논의: report 대화 로그 (균등 vs 스코어가중 vs ATR 3안 비교).

**결정: 저·중위험 = 균등가중, 고위험 = ATR 역가중. 둘 다 티어 밴드 클램프.**

1. **편입 종목 수** — `N = min(floor(카테고리_예산 / 티어_종목당_하한), max_positions[cat])`.
   종목당 하한: 저 3% / 중 3% / 고 1% (전체 포트 대비). 예산이 작으면 종목 수를 줄여
   각 포지션이 유의미하게 (하한 이상) 유지. `max_positions` (paper: 저20/중15/고15).
2. **저·중위험 — 균등가중**: `w_raw = 카테고리_예산 / N` → 티어 밴드 [저 3~6%, 중 3~5%]
   로 클램프 → 클램프 잔여는 밴드 여유 있는 종목에 비례 재분배 (1회). 스코어는
   **편입 커트라인 결정에만** 사용 (상위 N위). 편입 후 비중은 동일.
   근거: 적금형 회전율 최소화 (목표 비중은 편입/제외 변동 시에만 변함), 퍼센타일
   합성점수의 미세 서열 신뢰도 낮음, 백테스트 귀인 명확성.
3. **고위험 — ATR 역가중**: `w_raw ∝ 1 / (ATR14 / 종가)` (변동성 큰 종목 → 작은 포지션)
   → 정규화 × 카테고리_예산 → 밴드 [1~2.5%] 클램프 → 잔여 재분배.
   근거: 🔴 하드손절이 ATR 비례(-15~25%)이므로, ATR 역비례 사이징 → 손절 시
   종목당 달러 손실 균등(동일 리스크 기여). `indicators.atr` 재사용.
4. **고위험 예산 미달 처리**: N개 × 상한(2.5%)로도 예산을 못 채우면(종목 부족),
   미달분은 **현금으로 두지 않고 중위험 예산에 가산** (다음 우선순위 티어).
   반대로 넘치면 고위험 절대캡 20%가 상위 가드레일로 이미 차단.
5. **조직 레이어(3a-9+)**: ⑥ PM 에이전트가 이 균등/ATR 베이스라인 위에 ±3%p 재량
   틸트. 결정론 코어는 중립 베이스라인 유지가 역할 분담.
