...
- `prior_category_weights` 추가로 결정론 FAIL 이 나도 실행은 안 막음 (기존대로 리포트에만
  ...
  **개정 (2026-09-02, whole-integration #2 D옵션 + 후속 /code-review)**: `run_pipeline` 이
  **절대 가드레일** FAIL 시 raise (max_change 는 note+WARNING 유지, `prior` 는 `current_cat_usd/nav`
  그대로). reviews/whole-integration.
